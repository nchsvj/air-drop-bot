...
def claim_airdrop(message: types.Message):
    ...
    try:
        ...
        # Если требуется капча, но она еще не пройдена
        if require_captcha and user_id not in captcha_attempts:
            # Генерируем капчу
...
def process_captcha(message: types.Message):
    ...
    if user_answer == correct_answer:
        ...
        bot.send_message(
            message.chat.id,
            "✅ Капча пройдена успешно! Теперь вы можете получить свой airdrop.",
            reply_markup=create_main_keyboard()
        )
        # Вызываем claim_airdrop снова для получения вопроса
        claim_airdrop(message)
    else:
        if captcha_attempts[user_id] >= 3:
            # Превышено количество попыток
            bot.send_message(
                message.chat.id,
                "❌ Превышено количество попыток ввода капчи. Попробуйте получить airdrop позже.",
                reply_markup=create_main_keyboard()
            )
            # Очищаем данные
            del user_captchas[user_id]
            del captcha_attempts[user_id]
            # Сбрасываем требование капчи
            try:
                cur.execute("""
                    UPDATE users 
                    SET require_captcha = FALSE
                    WHERE user_id = %s;
                """, (user_id,))
                conn.commit()
            except psycopg2.Error as e:
            ...
            bot.send_photo(
                message.chat.id,
                captcha_image,
                caption=f"Неверный код. Попробуйте еще раз (попытка {captcha_attempts[user_id]}/3):"
            )
